...
def main(graph):
    ...
    Pdeg = g.getDoubleProperty("degree")
    g.applyDoubleAlgorithm("Degree", Pdeg)
    ...
    with open('degree.csv','w') as f:
        ...
        for i in range (100):
            node = Pdeg.getNodesEqualTo(Pdeg.getNodeMax()).next()
            g.delNode(node)
            ...
            writer.writerow([i,'degree',maxl/g.numberOfNodes(),tlp.averagePathLength(g)])
