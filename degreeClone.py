...
def main(graph):
    gDeg = graph.addCloneSubGraph("Degree")
    
    gPageRank = graph.addCloneSubGraph("Page Rank")
    gBetweenness = graph.addCloneSubGraph("Betweenness Centrality")
    
    betweennessDs = tlp.getDefaultPluginParameters("Betweenness Centrality", gBetweenness)
    ...
    allCentralities =[ 
         { 'Name':'Degree',  'max':True, 'graph': gDeg, 'currentAvgPath':0,'currLengthComponent':0 },
        ...
        #{ 'Name':'Eccentricity', 'max':False, 'graph':gEccentricity , 'currentAvgPath':0,'currLengthComponent':0 },
         { 'Name':'Page Rank',  'max':True, 'graph': gPageRank, 'currentAvgPath':0,'currLengthComponent':0 }
    ]
    
    with open('degreeClone.csv','w') as f:

        writer= csv.writer(f)
        writer.writerow(['nb_sommets',
            'mesure#1','taille_composante#1','lg moyenne#1',
            'mesure#2','taille_composante#2','lg moyenne#2',
            'mesure#3','taille_composante#3','lg moyenne#3',
            'mesure#4','taille_composante#4','lg moyenne#4',
            'mesure#5','taille_composante#5','lg moyenne#5'])
        ...
        for i in range (100):
            for centrality in allCentralities:
                ...
                if("ds" in centrality):
                    ...
                    currGraph.applyDoubleAlgorithm(centrality['Name'],cent,betweennessDs)
                else:
